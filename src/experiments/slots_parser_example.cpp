...
#include "nlohmann/borzoi_receive_tetra_slots.hpp" // IWYU pragma: keep
#include <cstddef>
#include <cstdlib>
#include <cxxopts.hpp>
#include <filesystem>
...
auto main(int argc, char** argv) -> int {
    ...
    cxxopts::Options options("slots-parser-example", "Reads a csv file and extracts the contained slots.");

    // clang-format off
	options.add_options()
		("h,help", "Print usage")
		("datafile", "the path to the file containing historic slots data", cxxopts::value<std::string>(datafile)->default_value("datafile.csv"))
		;
    // clang-format on
    ...
    for (std::size_t i = 0; i < doc.GetRowCount(); i++) {
        ...
        for (std::size_t i = 0; i < row.size(); i++) {
            ...
            if (column == "first_slot_data" || (column == "second_slot_data" && !value.empty())) {
        ...
        BorzoiReceiveTetraSlots packet = json;
